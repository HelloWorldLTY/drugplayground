...
ESM_PKL_PATH = "/data/zf/data/LTY-Proj/esm_emb/data_Human.pkl"  # Key: target_sequence
UNIMOL_PKL_PATH = "/data/zf/data/LTY-Proj/esm_unimol/unimol_data_Human.pkl"  # Key: compound_iso_smiles
CSV_PATH = "/data/zf/data/LTY-Proj/label/Human.csv"
...
esm_embeddings = load_pkl_dict(ESM_PKL_PATH)  # {target_sequence: embedding}
print("Loading UniMol embeddings...")
unimol_embeddings = load_pkl_dict(UNIMOL_PKL_PATH)  # {compound_iso_smiles: embedding}
...
for i, row in csv_data.iterrows():
    ...
    compound_smiles = str(row['compound_iso_smiles']).strip()
    target_seq = str(row['target_sequence']).strip()
...
if skipped_samples:
    ...
    print(f"  Missing target_sequence: {missing_seq}")
...
EPOCHS = 50
NUM_FOLDS = 5
RESULTS_FILE = "Human_5_fold_results.txt"
...
with open(RESULTS_FILE, "w") as f:
    ...
    f.write("  1. Use compound_iso_smiles to find embedding in unimol_data_Human.pkl\n")
    f.write("  2. Use target_sequence to find embedding in esm_emb/data_Human.pkl\n")
    ...
    if skipped_samples:
        ...
        f.write(f"  Missing target_sequence: {missing_seq}\n")
